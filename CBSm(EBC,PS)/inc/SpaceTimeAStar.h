...
public:
	...
	AStarNode(int loc, int g_val, int h_val, LLNode* parent, int timestep, int num_of_conflicts = 0, int temp_previous_makespan=-1, bool in_openlist = false) :
		LLNode(loc, g_val, h_val, parent, timestep, num_of_conflicts, temp_previous_makespan, in_openlist) {}
...
private:
	...
	// find path
	Path findShortestPath(ConstraintTable& constraint_table, const pair<int, int> start_state);
    Path findShortestPath(ConstraintTable& constraint_table, const pair<int, int> start_state, const pair<int, int> landmark);
