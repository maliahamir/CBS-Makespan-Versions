							  ...
							  const vector<Path*>& paths, int agent, int lowerbound)
{
	...
	if (constraint_table.length_min >= MAX_TIMESTEP ||
		...
		constraint_table.constrained(start_location, 0)) // the agent cannot start at its start location
	{
		return Path();
	}
	...
	temp_previous_makespan = node.makespan;
	if (lowerbound == -5)
		temp_previous_makespan = -1;

	if (constraint_table.getNumOfLandmarks() > 0)
	{
		cout << "WE HAVE LANDMARKS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
		...
		for (const auto constraint : node.constraints)
		{
			...
			case  constraint_type::LEQLENGTH:
				//NOT SURE: LEQLENGTH CONSTRAINT FOR AGENT <a> IS A VERTEX CONSTRAINT FOR ALL THE OTHER AGENTS FROM TIME T TO INFINITY.
				if (agent != a)
				{
			...
			case constraint_type::POSITIVE_VERTEX:
				//POSITIVE VERTEX CONSTRAINT FOR AGENT <a> IS A VERTEX CONSTRAINT FOR ALL THE OTHER AGENTS.
				if (agent != a)
				{
			...
			case constraint_type::POSITIVE_EDGE:
				//POSITIVE EDGE CONSTRAINT FOR AGENT <a> IS 2 VERTEX AND 1 EDGE CONSTRAINTS FOR ALL THE OTHER AGENTS.
				if (agent != a)
				{
		...
		if (goal_state.second == MAX_TIMESTEP)
		{
			...
			temp_previous_makespan = node.makespan - start_state.second;
			if (lowerbound == -5)
				temp_previous_makespan = -1;
			auto path_segment = findShortestPath(constraint_table, start_state);
		...
		else if ((int)paths[agent]->size() <= goal_state.second) // the landmark is at the goal location after the agent reaches
        {
			...
			if (lowerbound == -5)
				temp_previous_makespan = -1;
            auto path_segment = findShortestPath(constraint_table, start_state, goal_state);
            if (path_segment.empty())
            {
                // assert(test_path.empty());
                return Path();
		...
		if (lowerbound == -5)
			temp_previous_makespan = -1;
		return findShortestPath(constraint_table, make_pair(start_location, 0));
}
...
Path SpaceTimeAStar::findShortestPath(ConstraintTable& constraint_table, const pair<int, int> start_state)
{
	...
	lower_bound = max(holding_time - start_state.second, max(min_f_val, temp_previous_makespan));
	start->temp_previous_makespan = lower_bound; // SURE??
	while (!open_list.empty())
	{
		updateFocalList(); // update FOCAL if min f-val increased
		
		//curr is the node with the lowest f-value and with tie-breaking of lowest number of conflicts.
		auto* curr = popNode();
		
		// vector<AStarNode*> elements;
		// int size_f = focal_list.size();
		// cout << "curr g: " << curr->g_val << " curr h: " << curr->h_val << " curr C: " << curr->temp_previous_makespan << endl;
		// for (int i=0;i<size_f and i<2; i++)
		// {
		// 	auto* temp = focal_list.top();
		// 	cout << "temp g: " << temp->g_val << " temp h: " << temp->h_val << " temp C: " << temp->temp_previous_makespan << endl;
		// 	elements.push_back(focal_list.top());
		// 	focal_list.pop();
		// }
		// for (int i=0;i<size_f and i<2; i++)
		// {
		// 	focal_list.push(elements.at(i));
		// }
		// elements.clear();
		// elements.resize(0);
		...
		next_locations.emplace_back(curr->location); //I think this adds the wait move
		for (int next_location : next_locations)
		{
			int next_timestep = curr->timestep + 1;
			//CHECK IF EVERYTHING IS STATIC FROM NOW ON.
			if (max((int) constraint_table.cat_size, constraint_table.latest_timestep) + 1 < curr->timestep)
			{
				//if from curr.timestep we don't have other paths or constraints anymore then it's useless to use wait moves.
				if (next_location == curr->location)
				{
					continue;
				}
				//Return the next_timestep value to the curr_timestep because we don't need this value anymore
				//(And maybe we don't want it to get bigger and take more resources???).
				next_timestep--;
			}

			//If the move destination or edge have constraints then discard that move
			...
			//get the number of conflicts of the path from start to next_location
			//(each step can add at most 1 conflict)
			...
			auto next = new AStarNode(next_location, next_g_val, next_h_val,
									  curr, next_timestep, next_internal_conflicts, lower_bound, false); //SURE??
...
Path SpaceTimeAStar::findShortestPath(ConstraintTable& constraint_table, const pair<int, int> start_state, const pair<int, int> landmark)
{
...
{
	cout << "WIERD TYPE OF SEARCH ####################################################" << endl;
	// generate start and add it to the OPEN & FOCAL list
	Path path;
	auto start = new AStarNode(start_state.first,  // location
		0,  // g val 
	...
	while (!focal_list.empty())
	{
		...
		for (int next_location : next_locations)
		{
			...
			// generate (maybe temporary) node
			auto next = new AStarNode(next_location, next_g_val, next_h_val,
				curr, next_timestep, next_internal_conflicts, temp_previous_makespan, false);
...
//only used for corridor reasoning.
...
//pop from focal (and therefore from open)
...
//push to open (and if allowed to enter to focal then push to focal too)
...
//if the minimum f value had been increased
//then update the lower bound and the min_f_val
//and push to focal new nodes that are allowed to enter it.
void SpaceTimeAStar::updateFocalList()
{
	...
	if (open_head->getFVal() > min_f_val)
	{
		...
		int new_lower_bound = max(lower_bound, max(new_min_f_val, temp_previous_makespan));
		...
		if (lower_bound != new_lower_bound)
		{
			// for (auto n : open_list)
			// {
			// 		n->temp_previous_makespan = new_lower_bound;
			// }
			// cout << "1" << endl;
			int focal_list_size = focal_list.size();
			for (int i = 0; i< focal_list_size; i++)
			{
				focal_list.pop();
			}
			// cout << "size: " << focal_list.size() << endl;
			// cout << "2" << endl;
			for (auto n : open_list)
			{
				n->temp_previous_makespan = new_lower_bound;
			...
			// cout << "3" << endl;
			// cout << "makespan: " << temp_previous_makespan << endl;
			// cout << "old lower bound: " << lower_bound << endl;
			// cout << "new lower bound: " << new_lower_bound << endl;
		}
		lower_bound = new_lower_bound;
	}
}

//clean the Astar search memory
