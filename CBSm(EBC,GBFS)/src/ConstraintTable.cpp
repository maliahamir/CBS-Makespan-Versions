...
int ConstraintTable::getNumOfConflictsForStep(size_t curr_id, size_t next_id, int next_timestep) const
{
	if (map_size < map_size_threshold)
	{
		...
		//AMIR: if i move to an occupied location or swap with someone, return 1
		//AMIR: BUT!!! this also counts movements like 1,2,3 ->2,3,4 which are not conflicts (BUG?).
...
// return the earliest timestep that the agent can hold its goal location (stay there forever)
int ConstraintTable::getHoldingTime()
{
	...
	for (auto landmark : landmarks)
	{
		if (landmark.second != goal_location)
			rst = max(rst, (int) landmark.first + 1); //AMIR: why not "+ ManhattanDistance(landmark,goal)" instead of "+ 1".
	}
	return rst;
}
