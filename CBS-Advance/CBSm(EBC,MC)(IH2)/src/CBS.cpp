...
void CBS::classifyConflicts(CBSNode& node)
{
	...
	while (!node.unknownConf.empty())
	{
		...
		{
			//cout << "1: " << con->constraint1.back() << endl;
			mdd_helper.findSingletons(node, a1, *paths[a1]);
			//cout << "2: " << con->constraint1.back() << endl;
		}
		...
		child[0]->constraints = con->constraint1;
		child[1]->constraints = con->constraint2;
		if (con->type == conflict_type::RECTANGLE && rectangle_helper.strategy == rectangle_strategy::DR)
		{
			int i = (bool)(rand() % 2);
		...
		else if (con->type == conflict_type::CORRIDOR && corridor_helper.getStrategy() == corridor_strategy::DC)
		{
			int i = (bool)(rand() % 2);
		...
		bool solved1 = generateChild(child[0], &node);
		updatePaths(&node);
		bool solved2 = generateChild(child[1], &node);
		updatePaths(&node);
		...
		//SOC cardinality
		// if (child[0]->g_val > node.g_val || !solved1)
		// {
		// 	cardinal1 = true;
		// }
		// if (child[1]->g_val > node.g_val || !solved2)
		// {
		// 	cardinal2 = true;
		// }
		//Makespan cardinality
		if (child[0]->makespan > node.makespan || !solved1)
		{
			cardinal1 = true;
		}
		if (child[1]->makespan > node.makespan || !solved2)
		{
			cardinal2 = true;
		}
		...
		/*if (cardinal1 != cardinal1_new || cardinal2 != cardinal2_new)
		{
			cout << "SIMULATION IS WRONG:\n" << cardinal1 << cardinal1_new << cardinal2 << cardinal2_new << endl;
		}*/
		//Simulation
...
inline void CBS::pushNode(CBSNode* node)
{
	...
	if (node->makespan <= focal_list_threshold)
...
void CBS::updateFocalList()
{
	...
	if (open_head->makespan > min_f_val)
	{
		if (screen == 3)
		{
			cout << "  Note -- FOCAL UPDATE!! from |FOCAL|=" << focal_list.size() << " with |OPEN|=" << open_list.size() << " to |FOCAL|="; //TODO
		}
		min_f_val = open_head->makespan;
		double new_focal_list_threshold = min_f_val * focal_w;
		for (CBSNode* n : open_list)
		{
			if (n->makespan > focal_list_threshold &&
				n->makespan <= new_focal_list_threshold)
...
void CBS::printResults() const
{
	...
	cout << "SOC cost:" << solution_cost << ", makespan cost:" << solution_makespan_cost << ", runtime:" << runtime << ", HL nodes expanded:" <<
		 num_HL_expanded << ", LL nodes expanded:" << num_LL_expanded << ", LL runs:" << num_LL_runs << ", min_f_val:" << // HL_num_generated << "," << LL_num_generated << "," <<
		 min_f_val << ", dummy_start->g_val:" << dummy_start->g_val << ", dummy_start->g_val + dummy_start->h_val:" << dummy_start->g_val + dummy_start->h_val << "," <<
		 endl;
}
...
void CBS::saveResults(const string& fileName, const string& instanceName) const
{
	...
	if (!exist)
	{
		...
		addHeads << "#agents,runtime,#high-level expanded,#high-level generated,#low-level expanded,#low-level generated,#low-level runs,#root conflicts," <<
				 "solution cost,solution makespan cost," <<
				 //"sorted paths lengths," <<
				 "min f value,root makespan,root h value," <<
				 "makespan difference (solution makespan-root makespan),makespan difference with h (solution makespan-root makespan-root h),root g value,root f value," <<
				 "#adopt bypasses,root generation time," <<
	...
	// vector<int> sorted_paths_lengths(num_of_agents, -1);
	// int i = 0;
	// for (Path* path : paths)
	// {
	// 	sorted_paths_lengths[i] = path->size()-1;
	// 	i++;
	// }
	// std::sort(sorted_paths_lengths.begin(),sorted_paths_lengths.end(),greater<int>());
	// string sorted_paths_lengths_str = "";
	// for (size_t i=0; i<sorted_paths_lengths.size(); i++)
	// {
	// 	sorted_paths_lengths_str += to_string(sorted_paths_lengths[i]);
	// 	sorted_paths_lengths_str += ".";
	// }

	ofstream stats(fileName, std::ios::app);
	stats << num_of_agents << "," << runtime << "," <<
		  num_HL_expanded << "," << num_HL_generated << "," <<
		  num_LL_expanded << "," << num_LL_generated << "," << num_LL_runs << "," << dummy_start->number_of_conflicts << "," <<

		  solution_cost << "," << solution_makespan_cost << "," <<
		  //sorted_paths_lengths_str << "," <<
		  min_f_val << "," <<
		  dummy_start->makespan << "," << dummy_start->h_val << "," << solution_makespan_cost-dummy_start->makespan << "," <<
		  solution_makespan_cost-dummy_start->makespan-dummy_start->h_val << "," << dummy_start->g_val << "," << dummy_start->g_val + dummy_start->h_val << "," <<

		  num_adopt_bypass << "," << root_generation_time << "," <<
...
bool CBS::solve(double _time_limit, int _cost_lowerbound, int _cost_upperbound)
{
	...
	root_generation_time = (double) (clock() - start) / CLOCKS_PER_SEC;
	...
	while (!open_list.empty() && !solution_found)
	{
		...
		{  // time/node out
			solution_cost = -1;
			solution_makespan_cost = -1;
			solution_found = false;
			break;
		}

		/*CBSNode* tempNode = NULL;
		int y = allNodes_table.size();
		list<CBSNode*> temp_all_nodes_table;
		for(int i=0; i<y; i++)
		{
			tempNode = allNodes_table.front();
			allNodes_table.pop_front();
			cout << endl << "High level node " << i << " f_val:" << tempNode->g_val+tempNode->h_val << ", makespan:" << tempNode->makespan << ", number of conflicts:" << tempNode->conflicts.size() << ", number of unknown conflicts:" << tempNode->unknownConf.size();
			for(int tempAgent=0; tempAgent<num_of_agents; tempAgent++)
			{
				cout << endl << "constraints of agent " << tempAgent << ": ";
				tempNode->printConstraints(tempAgent);
			}
			cout << endl;
			printConflicts(*tempNode);
			cout << endl;
			temp_all_nodes_table.push_back(tempNode);
		}
		allNodes_table = temp_all_nodes_table;
		CBSNode* currTemp = NULL;
		int x = focal_list.size();
		pairing_heap< CBSNode*, compare<CBSNode::secondary_compare_node>> temp_focal_list;
		for(int i=0; i<x; i++)
		{
			currTemp = focal_list.top();
			focal_list.pop();
			cout << endl << "focal list " << i << " f_val:" << currTemp->g_val+currTemp->h_val << ", makespan:" << currTemp->makespan << ", tie breaking value:" << currTemp->tie_breaking << ", number of conflicts:" << tempNode->conflicts.size() << ", number of unknown conflicts:" << tempNode->unknownConf.size();
			for(int tempAgent=0; tempAgent<num_of_agents; tempAgent++)
			{
				cout << endl << "constraints of agent " << tempAgent << ": ";
				currTemp->printConstraints(tempAgent);

			}
			cout << endl;
			printConflicts(*currTemp);
			cout << endl;
			temp_focal_list.push(currTemp);
		}
		focal_list = temp_focal_list;*/
		...
		{// found a solution (and finish the while look)
			...
			solution_makespan_cost = curr->makespan;
			goal_node = curr;
			break;
		}
		...
		if (!curr->h_computed) // heuristics has not been computed yet
		{
			...
			if (curr->makespan <= focal_list_threshold)
		...
		while (foundBypass)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				if (child[i]->makespan == min_f_val && curr->unknownConf.size() + curr->conflicts.size() == 0) //no conflicts
				{// found a solution (and finish the for loop)
					break;
				}
...
bool CBS::generateRoot()
{
	...
	dummy_start->g_val = 0;
	paths.resize(num_of_agents, nullptr); //num_of_agents = paths.size, nullptr = paths' initial values.
	...
	if (paths_found_initially.empty())
	{
		...
		// find shortest path length for each agent.
		...
		{
			num_LL_runs++;
			paths_found_initially[i] = search_engines[i]->findPath(*dummy_start, initial_constraints[i], paths, i, -5);
		...
		vector<int> agents_shortest_paths_lengths(num_of_agents);
		for (int i = 0; i < num_of_agents; i++)
		{
			auto spli = paths_found_initially[i].size()-1;
			agents_shortest_paths_lengths[i] = spli;
		}
		
		//sort the agents according to a given array (shortest paths lengths)
		vector<int> indices(num_of_agents);
		for (int i = 0; i < num_of_agents; i++)
		{
			indices[i] = i;
		}
		sort(indices.begin(), indices.end(), [&](int i, int j) {
			return agents_shortest_paths_lengths[i]  > agents_shortest_paths_lengths[j]; 
		});
		vector<int> sorted_agents(num_of_agents);
		for (int i = 0; i < num_of_agents; i++)
		{
			sorted_agents[i]  = agents[indices[i]];
		}
		...
		{
			num_LL_runs++;
			paths_found_initially[i] = search_engines[i]->findPath(*dummy_start, initial_constraints[i], paths, i, -4);
			if (paths_found_initially[i].empty())
			{
				cout << "No path exists for agent " << i << endl;
				return false;
	...
	min_f_val = max(min_f_val, (double) dummy_start->makespan);
