...
inline void CBS::pushNode(CBSNode* node)
{
	...
	if (node->makespan+node->h_val <= focal_list_threshold)
...
void CBS::updateFocalList()
{
	...
	if (open_head->makespan+open_head->h_val > min_f_val)
	{
		if (screen == 3)
		{
			cout << "  Note -- FOCAL UPDATE!! from |FOCAL|=" << focal_list.size() << " with |OPEN|=" << open_list.size() << " to |FOCAL|="; //TODO
		}
		min_f_val = open_head->makespan + open_head->h_val;
		double new_focal_list_threshold = min_f_val * focal_w;
		for (CBSNode* n : open_list)
		{
			if (n->makespan + n->h_val > focal_list_threshold &&
				n->makespan + n->h_val <= new_focal_list_threshold)
...
void CBS::saveResults(const string& fileName, const string& instanceName) const
{
	...
	if (!exist)
	{
		...
		addHeads << "#agents,runtime,#high-level expanded,#high-level generated,#low-level expanded,#low-level generated,#low-level runs," <<
				 "solution cost,solution makespan cost,sorted paths lengths,min f value,root makespan,root h value," <<
	...
	stats << num_of_agents << "," << runtime << "," <<
		  ...
		  num_LL_expanded << "," << num_LL_generated << "," << num_LL_runs << "," <<

		  solution_cost << "," << solution_makespan_cost << "," << sorted_paths_lengths_str << "," << min_f_val << "," <<
...
bool CBS::solve(double _time_limit, int _cost_lowerbound, int _cost_upperbound)
{
	...
	while (!open_list.empty() && !solution_found)
	{
		...
		if (!curr->h_computed) // heuristics has not been computed yet
		{
			...
			if (curr->makespan + curr->h_val <= focal_list_threshold)
		...
		while (foundBypass)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				if (child[i]->makespan == min_f_val && child[i]->h_val == 0 && curr->unknownConf.size() + curr->conflicts.size() == 0) //no conflicts
...
bool CBS::generateRoot()
{
	...
	min_f_val = max(min_f_val, (double) dummy_start->makespan + dummy_start->h_val);
