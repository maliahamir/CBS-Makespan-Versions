...
int main(int argc, char** argv)
{
	...
	desc.add_options()
		...
		//("agentNum,k", po::value<int>()->default_value(0), "number of agents") ONE RUN
		("agentNum,k", po::value<string>()->required(), "number of agents") // ONE RUN
		...
		// params for CBS
		("heuristics", po::value<string>()->default_value("WDG"), "heuristics for the high-level search (Zero, CG,DG, WDG, M2)")
	...
	else if (vm["heuristics"].as<string>() == "M2")
		h = heuristics_type::M2;
	else
	{
	...
	vector<string> scens;
	string scen;
	std::stringstream ss1(vm["agents"].as<string>());
	while(getline(ss1,scen,','))
	{
		scens.push_back(scen);
	}

	vector<int> agentsNumbers;
	string numberOfAgents;
	std::stringstream ss2(vm["agentNum"].as<string>());
	while(getline(ss2,numberOfAgents,','))
	{
		agentsNumbers.push_back(stoi(numberOfAgents));
	}

	for (int agentsNumber: agentsNumbers)
	{
		for (string scenario: scens)
		{
			cout << scenario << " with " << agentsNumber << " agents" << endl;
			Instance instance(vm["map"].as<string>(), scenario, agentsNumber, vm["agentIdx"].as<string>(), vm["rows"].as<int>(), vm["cols"].as<int>(), vm["obs"].as<int>(), vm["warehouseWidth"].as<int>());
			...
			//////////////////////////////////////////////////////////////////////
			if (vm.count("output"))
				cbs.saveResults(vm["output"].as<string>(), scenario+":"+ vm["agentIdx"].as<string>());
			// cbs.saveCT(vm["output"].as<string>() + ".tree"); // for debug
			if (vm["stats"].as<bool>())
			{
				cbs.saveStats(vm["output"].as<string>(), scenario + ":" + vm["agentIdx"].as<string>());
			}
