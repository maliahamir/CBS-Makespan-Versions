...
{
	node.h_val = max(0, node.parent->g_val + node.parent->h_val - node.g_val); // pathmax: (curr heuristic) = (parent heuristic) - (the edge weight between us).
...
bool CBSHeuristic::computeInformedHeuristics(CBSNode& curr, double _time_limit)
{
	...
	case heuristics_type::M2:
		h = findMakespanPairsHeuristics(curr);
		
		// if (h>0 && h<1000)
		// {
		// 	cout << "Node: " << curr.time_generated << ", h: " << h << endl;
		// }
		break;
	}
	if (h < 0)
...
int CBSHeuristic::findMakespanPairsHeuristics(CBSNode& node)
{
	int makespan_lower_bound = node.makespan;
	vector<bool> pairFlags(num_of_agents * num_of_agents, false);
	for (const auto& conflict : node.conflicts)
	{
		...
		if (got != lookupTable[a1][a2].end()) // check the lookup table first
		{
			num_memoization++;
			int x = got->second;
			makespan_lower_bound = max(makespan_lower_bound, x);
		}
		else
		{
			int AgentsPairMakespan = solve2AgentsMakespan(a1, a2, node);
			makespan_lower_bound = max(makespan_lower_bound, AgentsPairMakespan);
			lookupTable[a1][a2][DoubleConstraintsHasher(a1, a2, &node)] = AgentsPairMakespan;
		}
		pairFlags[idx] = true;
	}
	for (const auto& conflict : node.unknownConf)
	...
	return makespan_lower_bound - node.makespan;
}
...
int CBSHeuristic::solve2AgentsMakespan(int a1, int a2, const CBSNode& node)
{
	...
	int root_makespan = max((int)initial_paths[0].size() - 1, (int)initial_paths[1].size() - 1); //makespan
	int lowerbound = root_makespan;
	...
	if (cbs.runtime > time_limit - runtime || cbs.num_HL_expanded > node_limit) // time out or node out
		rst = (int)cbs.min_f_val; // using lowerbound to approximate
	else if (cbs.solution_cost  < 0) // no solution
		rst = MAX_COST;
	else
	{
		rst = cbs.solution_makespan_cost;
	}
	if (save_stats)
	{
...
int CBSHeuristic::solve2Agents(int a1, int a2, const CBSNode& node, bool cardinal)
{
	...
	double runtime = (double)(clock() - start_time) / CLOCKS_PER_SEC;
	int root_g = (int)initial_paths[0].size() - 1 + (int)initial_paths[1].size() - 1; //sum_of_cost
...
int CBSHeuristic::MVConAllConflicts(CBSNode& curr)  //nothing is using this.
{
...
vector<int> CBSHeuristic::buildConflictGraph(const CBSNode& curr) const //nothing is using the only thing that is using this.
{
