...
enum heuristics_type { ZERO, CG, DG, WDG, M2, STRATEGY_COUNT };
...
public:
	...
	uint64_t num_memoization = 0; // number of times when memeoization helps
	...
	void init()
	{
		if (type == heuristics_type::DG || type == heuristics_type::WDG || type == heuristics_type::M2)
		{
...
private:
	...
	vector<int> buildConflictGraph(const CBSNode& curr) const; //not in use
	...
	int solve2AgentsMakespan(int a1, int a2, const CBSNode& node);
	...
	int findMakespanPairsHeuristics(CBSNode& node);
	...
	int MVConAllConflicts(CBSNode& curr); //not in use
};