...
bool CBS::generateRoot()
{
	...
	if (paths_found_initially.empty())
	{
		...
		vector<int> agents_manhattan(num_of_agents);
		for (int i = 0; i < num_of_agents; i++)
		{
			auto x = search_engines[i];
			auto mdi = x->instance.getManhattanDistance(x->start_location,x->goal_location);
			agents_manhattan[i] = mdi;
		}
		...
		sort(indices.begin(), indices.end(), [&](int i, int j) {
			return agents_manhattan[i]  > agents_manhattan[j]; 
		});
