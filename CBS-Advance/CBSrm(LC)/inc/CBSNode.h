...
public:
	...
	struct secondary_compare_node 
	{
		bool operator()(const CBSNode* n1, const CBSNode* n2) const 
		{
			int number_of_agents = n1->num_of_agents;
			//get the nodes paths lengths sorted.
			vector<bool> updated1(number_of_agents, false);
			vector<int> lengths1(number_of_agents, -1);
			while (n1 != nullptr)
			{
				for (auto & path : n1->paths)
				{
					if (!updated1[path.first])
					{
						lengths1[path.first] = path.second.size();
						updated1[path.first] = true;
					}
				}
				n1 = n1->parent;
			}

			vector<bool> updated2(number_of_agents, false);
			vector<int> lengths2(number_of_agents, -1);
			while (n2 != nullptr)
			{
				for (auto & path : n2->paths)
				{
					if (!updated2[path.first])
					{
						lengths2[path.first] = path.second.size();
						updated2[path.first] = true;
					}
				}
				n2 = n2->parent;
			}

			std::sort(lengths1.begin(),lengths1.end(),greater<int>());
			std::sort(lengths2.begin(),lengths2.end(),greater<int>());

			//check if the makespan2 of n1 is better than of n2.
			//if it is the same, then check for makespan3, and so on...
			...
			for (size_t i=0; i<lengths1.size(); i++)
			{
				if (lengths1[i] < lengths2[i])
				{
					//n1 is better
					// cout << i << endl;
					// cout << "n1 should be better" << endl;
					// cout << "n1: ";
					// for(int x=0; x<lengths1.size(); x++)
					// 	cout << lengths1[x] << ", ";
					// cout << endl << "n2: ";
					// for(int x=0; x<lengths2.size(); x++)
					// 	cout << lengths2[x] << ", ";
					// cout << endl;
					return false;
				}
				else if (lengths1[i] > lengths2[i])
				{
					//n2 is better
					// cout << i << endl;
					// cout << "n2 should be better" << endl;
					// cout << "n1: ";
					// for(int x=0; x<lengths1.size(); x++)
					// 	cout << lengths1[x] << ", ";
					// cout << endl << "n2: ";
	...
	typedef boost::heap::pairing_heap<CBSNode*, boost::heap::compare<CBSNode::makespan_compare_node>>::handle_type open_handle_t;
