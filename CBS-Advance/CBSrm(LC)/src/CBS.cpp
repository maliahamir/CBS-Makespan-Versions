...
void CBS::classifyConflicts(CBSNode& node)
{
	...
	while (!node.unknownConf.empty())
	{
		...
		bool cardinal1_new = false;
		bool cardinal2_new = false;
		...
		if (cardinal1_new && cardinal2_new)
		{
			con->priority = conflict_priority::CARDINAL;
		}
		else if (cardinal1_new || cardinal2_new)
		{
...
inline void CBS::pushNode(CBSNode* node)
{
	...
	node->num_of_agents = num_of_agents;
...
void CBS::printResults() const
{
	...
	cout << "SOC cost:" << solution_cost << ", makespan cost:" << solution_makespan_cost << ", runtime:" << runtime << ", HL nodes:" <<
		 num_HL_expanded << ", LL nodes:" << num_LL_expanded << ", LL runs:" << num_LL_runs << ", min_f_val:" << // HL_num_generated << "," << LL_num_generated << "," <<
...
void CBS::saveResults(const string& fileName, const string& instanceName) const
{
	...
	if (!exist)
	{
		...
		addHeads << "#agents,runtime,#high-level expanded,#high-level generated,#low-level expanded,#low-level generated,#low-level runs,#low-level expanded per low-level run," <<
				 "solution cost,solution makespan cost,sorted paths lengths,min f value,root g value,root f value," <<
				 "#adopt bypasses," <<
	...
	stats << num_of_agents << "," << min(runtime,time_limit) << "," <<
		  num_HL_expanded << "," << num_HL_generated << "," <<
		  num_LL_expanded << "," << num_LL_generated << "," << num_LL_runs << "," << ((float)(num_LL_expanded))/((float)(num_LL_runs)) << "," <<

		  solution_cost << "," << solution_makespan_cost << "," << sorted_paths_lengths_str << "," << min_f_val << "," << dummy_start->g_val << "," << dummy_start->g_val + dummy_start->h_val << "," <<
...
bool CBS::solve(double _time_limit, int _cost_lowerbound, int _cost_upperbound)
{
	...
	while (!open_list.empty() && !solution_found)
	{
		...
		// {
		// 	cout << "Node: " << x->time_generated << endl;
		// 	for (auto & y : x->paths)
		// 	{
		// 		cout << "agent: " << y.first << endl << "Path: " << y.second.size() << endl;
		// 	}
		// 	cout << endl;
		// }
		// cout << "END OPEN" << endl;

		// for (auto & x : focal_list)
		// {
		// 	cout << "Node: " << x->time_generated << endl;
		// 	for (auto & y : x->paths)
		// 	{
...
bool CBS::generateRoot()
{
	...
	dummy_start->num_of_agents = num_of_agents;
	...
	{
		Path w = *paths[x];
		dummy_start->paths.emplace_back(x,w);
	}
